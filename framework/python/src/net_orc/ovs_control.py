...
class OVSControl:
  ...
  def add_dhcp_filters(self,dhcp_server_primary_ip,dhcp_server_secondary_ip):
    ...
    allow_primary_dhcp_server = (
      f'''table=0, dl_type=0x800, priority=65535,
      tp_src=67, tp_dst=68, nw_src={dhcp_server_primary_ip}, actions=normal''')
    ...
    allow_secondary_dhcp_server = (
      f'''table=0, dl_type=0x800, priority=65535, tp_src=67,
      tp_dst=68, nw_src={dhcp_server_secondary_ip}, actions=normal''')
    ...
    drop_dhcp_flow = '''table=0, dl_type=0x800, priority=0, tp_src=67,
      tp_dst=68, actions=drop'''
  ...
  def add_arp_inspection_filter(self,ip_address,mac_address):
    ...
    allow_known_arps= f'''table=0, cookie={DEVICER_ARP_COOKIE}, priority=65535,
      arp, arp_tpa={ip_address}, arp_tha={mac_address}, action=normal'''
    ...
    dhcp_1_arps= f'''table=0, priority=65535, arp, arp_tpa={dhcp1_ip},
      arp_tha={dhcp1_mac}, action=normal'''
    dhcp_2_arps= f'''table=0, priority=65535, arp, arp_tpa={dhcp2_ip},
      arp_tha={dhcp2_mac}, action=normal'''
