...
class NetworkRunner:
  """Entry point to the Network Orchestrator."""

  def __init__(self, config_file=None, validate=True, async_monitor=False):
    self._monitor_thread = None
    self._register_exits()
    self.net_orc = NetworkOrchestrator(config_file=config_file,
                                       validate=validate,
                                       async_monitor=async_monitor)
  ...
  def _exit_handler(self, signum, arg):  # pylint: disable=unused-argument
    ...
    if signum in (2, signal.SIGTERM):
      ...
      # and don't care about a graceful shutdown
      self.stop(True)
      sys.exit(1)
...
if __name__ == "__main__":
  ...
  runner = NetworkRunner(config_file=arguments.config_file,
              validate=not arguments.no_validate,
              async_monitor=arguments.daemon)
  runner.start()
