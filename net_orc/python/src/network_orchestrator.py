...
class NetworkOrchestrator:
  ...
  def _device_discovered(self, mac_addr):
    ...
    LOGGER.debug("Discovered device {}. Waiting for device to obtain IP".format(
        mac_addr))
    ...
    if device.ip_addr is None:
      LOGGER.info(
          "Timed out whilst waiting for {} to obtain an IP address".format(
              mac_addr))
      return

    LOGGER.info("Device with mac addr {} has obtained IP address {}".format(
  ...
  def _start_device_monitor(self, device):
    ...
    LOGGER.info("Monitoring device with mac addr {} for {} seconds".format(
        device.mac_addr, str(self._monitor_period)))
  ...
  def _ci_post_network_create(self):
    ...
    util.run_command(
        "ip route add default via {} src {} metric 100 dev {}".format(
            self._gateway, self._ipv4[:-3], INTERNET_BRIDGE))
  ...
  def _load_network_module(self, module_dir):
    ...
    net_module_json = json.load(
        open(os.path.join(self._path, net_modules_dir, module_dir,
