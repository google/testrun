...
class NetworkValidator:
    ...
    def _build_device(self, net_device):
        ...
        try:
            ...
            client.images.build(
                dockerfile=os.path.join(net_device.dir, net_device.build_file),
                path=self._path,
                forcerm=True,
    ...
    def _load_devices(self):
        ...
        LOGGER.info(f"Loading validators from {DEVICES_DIR}")
        ...
        for module_dir in os.listdir(self._device_dir):
            ...
            if "enable_container" in device_json['config']['docker']:
                device.enable_container = device_json['config']['docker']['enable_container']
    ...
    def _attach_device_to_network(self, device):
        ...
        container_pid = util.run_command(
            "docker inspect -f {{.State.Pid}} " + device.container_name)[0]
    ...
    def _get_device_container(self, net_device):
        LOGGER.debug("Resolving device container: " +
                     net_device.container_name)
        container = None
        try:
            client = docker.from_env()
            ...
            LOGGER.debug("Container " +
                         net_device.container_name + " not found")
        except Exception as e:
