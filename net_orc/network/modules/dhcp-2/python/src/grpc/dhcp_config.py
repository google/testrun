...
class DHCPConfig:
    ...
    def resolve_config(self):
        ...
        self.peer = DHCPFailoverPeer(conf)
    ...
    def resolve_subnets(self, conf):
        ...
        for subnet in subnets:
            ...
            self._subnets.append(dhcp_subnet)

    def set_range(self, start, end, subnet=0, pool=0):
        print("Setting Range for pool ")
        print(self._subnets[subnet]._pools[pool])
        self._subnets[subnet]._pools[pool]._range_start = start
        self._subnets[subnet]._pools[pool]._range_end = end

    def resolve_settings(self, conf):
        lines = conf.split("\n")
        for line in lines:
            if DEFAULT_LEASE_TIME_KEY in line:
                self._default_lease_time = line.strip().split(DEFAULT_LEASE_TIME_KEY)[
                    1].strip().split(";")[0]
    ...
    def __str__(self):
        ...
        config = config.format(length='multi-line',
                               DEFAULT_LEASE_TIME_KEY=DEFAULT_LEASE_TIME_KEY, DEFAULT_LEASE_TIME=self._default_lease_time
                               )
...
class DHCPFailoverPeer:
    ...
    def __str__(self):
        ...
        return config.format(length='multi-line',
                             FAILOVER_PEER_KEY=FAILOVER_PEER_KEY, FAILOVER_PEER=self.name,
                             ADDRESS_KEY=ADDRESS_KEY, ADDRESS=self.address,
                             PORT_KEY=PORT_KEY, PORT=self.port,
                             PEER_ADDRESS_KEY=PEER_ADDRESS_KEY, PEER_ADDRESS=self.peer_address,
                             PEER_PORT_KEY=PEER_PORT_KEY, PEER_PORT=self.peer_port,
                             MAX_RESPONSE_DELAY_KEY=MAX_RESPONSE_DELAY_KEY, MAX_RESPONSE_DELAY=self.max_response_delay,
                             MAX_UNACKED_UPDATES_KEY=MAX_UNACKED_UPDATES_KEY, MAX_UNACKED_UPDATES=self.max_unacked_updates,
                             MCLT_KEY=MCLT_KEY, MCLT=self.mclt,
                             SPLIT_KEY=SPLIT_KEY, SPLIT=self.split,
                             LOAD_BALANCE_MAX_SECONDS_KEY=LOAD_BALANCE_MAX_SECONDS_KEY, LOAD_BALANCE_MAX_SECONDS=self.load_balance_max_seconds
                             )
    ...
    def resolve_peer(self, conf):
        ...
        for line in lines:
            if line.startswith(FAILOVER_PEER_KEY) or len(peer) > 0:
                if(len(peer) <= 0):
                    self.name = line.strip().split(FAILOVER_PEER_KEY)[
                        1].strip().split("{")[0].split("\"")[1]
                peer += line+"\n"
                if PRIMARY_KEY in line:
                    self.primary = True
                elif ADDRESS_KEY in line and PEER_ADDRESS_KEY not in line:
                    self.address = line.strip().split(ADDRESS_KEY)[
                        1].strip().split(";")[0]
                elif PORT_KEY in line and PEER_PORT_KEY not in line:
                    self.port = line.strip().split(PORT_KEY)[
                        1].strip().split(";")[0]
                elif PEER_ADDRESS_KEY in line:
                    self.peer_address = line.strip().split(PEER_ADDRESS_KEY)[
                        1].strip().split(";")[0]
                elif PEER_PORT_KEY in line:
                    self.peer_port = line.strip().split(PEER_PORT_KEY)[
                        1].strip().split(";")[0]
                elif MAX_RESPONSE_DELAY_KEY in line:
                    self.max_response_delay = line.strip().split(MAX_RESPONSE_DELAY_KEY)[
                        1].strip().split(";")[0]
                elif MAX_UNACKED_UPDATES_KEY in line:
                    self.max_unacked_updates = line.strip().split(MAX_UNACKED_UPDATES_KEY)[
                        1].strip().split(";")[0]
                elif MCLT_KEY in line:
                    self.mclt = line.strip().split(MCLT_KEY)[
                        1].strip().split(";")[0]
                elif SPLIT_KEY in line:
                    self.split = line.strip().split(SPLIT_KEY)[
                        1].strip().split(";")[0]
                elif LOAD_BALANCE_MAX_SECONDS_KEY in line:
                    self.load_balance_max_seconds = line.strip().split(LOAD_BALANCE_MAX_SECONDS_KEY)[
                        1].strip().split(";")[0]
...
class DHCPSubnet:
    ...
    def __str__(self):
        ...
        config = config.format(length='multi-line',
                               ...
                               SUBNET_MASK_OPTION_KEY=SUBNET_MASK_OPTION_KEY, SUBNET_MASK_OPTION=self._subnet_mask,
                               BROADCAST_OPTION_KEY=BROADCAST_OPTION_KEY, BROADCAST_OPTION=self._broadcast,
                               ROUTER_OPTION_KEY=ROUTER_OPTION_KEY, ROUTER_OPTION=self._routers,
                               DNS_OPTION_KEY=DNS_OPTION_KEY, DNS_OPTION=self._dns_servers
                               )
    ...
    def resolve_subnet(self, subnet):
        ...
        for part in subnet_parts:
            if NTP_OPTION_KEY in part:
                self._ntp_servers = part.strip().split(NTP_OPTION_KEY)[
                    1].strip().split(";")[0]
            elif SUBNET_MASK_OPTION_KEY in part:
                self._subnet_mask = part.strip().split(SUBNET_MASK_OPTION_KEY)[
                    1].strip().split(";")[0]
            elif BROADCAST_OPTION_KEY in part:
                self._broadcast = part.strip().split(BROADCAST_OPTION_KEY)[
                    1].strip().split(";")[0]
            elif ROUTER_OPTION_KEY in part:
                self._routers = part.strip().split(ROUTER_OPTION_KEY)[
                    1].strip().split(";")[0]
            elif DNS_OPTION_KEY in part:
                self._dns_servers = part.strip().split(DNS_OPTION_KEY)[
                    1].strip().split(";")[0]
...
class DHCPPool:
    ...
    def __str__(self):
        ...
        return config.format(length='multi-line',
                             FAILOVER_KEY=FAILOVER_KEY, FAILOVER=self._failover_peer,
                             RANGE_KEY=RANGE_KEY, RANGE_START=self._range_start, RANGE_END=self._range_end,
                             )
    ...
    def resolve_pool(self, pool):
        ...
        for part in pool_parts:
            if FAILOVER_KEY in part:
                self._failover_peer = part.strip().split(
                    FAILOVER_KEY)[1].strip().split(";")[0].replace("\"", "")
            if RANGE_KEY in part:
                range = part.strip().split(RANGE_KEY)[
                    1].strip().split(";")[0]
                self._range_start = range.split(" ")[0].strip()
                self._range_end = range.split(" ")[1].strip()
