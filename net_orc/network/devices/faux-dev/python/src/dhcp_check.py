...
class DHCPValidator:
    ...
    def _resolve_dhcp_lease(self):
        ...
        while self._dhcp_lease is None:
            time.sleep(5)
            try:
                lease_file = open(DHCP_LEASE_FILE)
                ...
                if cur_lease is not None:
                    ...
                    for part in lease_parts:
                        ...
                        if part_clean.startswith(IP_ADDRESS_KEY):
                            self._dhcp_lease.ip_addr = part_clean[len(
                                IP_ADDRESS_KEY):-1].strip()
                        elif part_clean.startswith(DNS_OPTION_KEY):
                            self._dhcp_lease.dns_server = part_clean[len(
                                DNS_OPTION_KEY):-1].strip()
                        elif part_clean.startswith(GATEWAY_OPTION_KEY):
                            self._dhcp_lease.gateway = part_clean[len(
                                GATEWAY_OPTION_KEY):-1].strip()
                        elif part_clean.startswith(NTP_OPTION_KEY):
                            self._dhcp_lease.ntp_server = part_clean[len(
                                NTP_OPTION_KEY):-1].strip()
            except Exception:
