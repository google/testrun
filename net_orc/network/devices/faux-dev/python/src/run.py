...
class FauxDevice:
    ...
    def run_tests(self):
        ...
        # Run DHCP tests first since everything hinges on basic DHCP compliance first
    ...
    def generate_results(self):
        ...
        results = []
        results.append(self.generate_result("dhcp_lease", self.dhcp_validator.dhcp_lease_test))
        results.append(self.generate_result("dns_from_dhcp", self.dns_validator._dns_dhcp_server_test))
        results.append(self.generate_result("dns_resolution", self.dns_validator._dns_resolution_test))
        results.append(self.generate_result("gateway_default", self.gateway_validator._default_gateway_test))
        results.append(self.generate_result("ntp_sync", self.ntp_validator._ntp_sync_test))
...
def run(argv): # pylint: disable=unused-argument
    ...
    parser = argparse.ArgumentParser(description="Faux Device _validator",
