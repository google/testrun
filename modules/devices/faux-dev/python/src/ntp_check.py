...
"""Used to check if the NTP server is functioning as expected"""
import time
import logger
import util
...
class NTPValidator:
  ...
  def print_test_results(self):
    ...
    self.print_test_result("NTP sync test", self.ntp_sync_test)
  ...
  def check_ntp(self):
    ...
    if self._ntp_server is not None:
      ...
      while attempt < ATTEMPTS and not self.ntp_sync_test:
        attempt += 1
        if self.ping_ntp_server():
          self.sync_ntp()
        if not self.ntp_sync_test:
