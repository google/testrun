...
class NmapModule(TestModule):
    ...
    def _scan_tcp_with_script(self, script_name, ports=None):
        ...
        results_file = "/runtime/output/" + self._module_name + "-"+script_name+".log"
    ...
    def _scan_udp_ports(self, tests):
        ...
        nmap_results, err = util.run_command(
            "nmap -sU -sV -p " + port_list + " " + self._device_ipv4_addr)
        ...
        if nmap_results:
            ...
            if rows:
                for result in rows[1:-1]:  # Iterate skipping the header and tail rows
                    cols = result.split()
                    port = cols[0].split("/")[0]
                    # If results don't start with a a port number, it's likely a bleed over
                    # from previous result so we need to ignore it
                    if port.isdigit():
                        version = ""
                        if len(cols) > 3:
                        ...
                        port_result = {cols[0].split(
                            "/")[0]: {"state": cols[1], "service": cols[2], "version": version}}
